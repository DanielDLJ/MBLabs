...
import registeredCompany from '../../constants';
...
function Settings({navigation}) {
  const {company, logout, update} = useContext(AuthContext);
  ...
  const sendImage = async () => {
    try {
      let updateCompany = new FormData();
      const upload_body = {
      ...
      updateCompany.append('image', upload_body);
  ...
  useEffect(() => {
    console.log(api.defaults.baseURL + '/' + company?.image.replace('\\', '/'))
  }, []);
            ...
            <Image
              source={
                company?.image
                  ? {
                      uri:
                      api.defaults.baseURL + '/' + company?.image.replace('\\', '/'),
                    }
                  : require('../../../assets/img/avatar.jpg')
              }
              style={styles.image}
